...
class NotificationHelper:
    ...
    def notify_order_placed(
        ...
        if customer_id:
            customer_message = f"Your order <b>{tracking_number}</b> has been placed successfully. Total: <b>Rs.{format_amount(total_amount)}</b>"
        ...
        admin_message = f"New order <b>{tracking_number}</b> has been placed. Total: <b>Rs.{format_amount(total_amount)}</b>"
        admin_count = 0
    ...
    def notify_return_request_approved(
        ...
        # Notify customer
        customer_message = f"Your return request for order <b>{order_tracking_number}</b> has been approved. Refund amount: <b>Rs.{format_amount(refund_amount)}</b>"
        NotificationHelper.create_notification(session, customer_id, customer_message)
    ...
    def notify_withdrawal_requested(
        ...
        # Notify shop owner
        owner_message = f"Your withdrawal request for <b>Rs.{format_amount(amount)}</b> has been submitted and is pending approval."
        NotificationHelper.create_notification(session, shop.owner_id, owner_message)
        ...
        for admin in admin_users:
            admin_message = f"New withdrawal request from shop <b>{shop.name}</b> for <b>Rs.{format_amount(amount)}</b>."
            NotificationHelper.create_notification(
                session, admin.id, admin_message, commit=False
            )
    ...
    def notify_withdrawal_approved(session: Session, shop_id: int, amount: float):
        ...
        message = f"<b>Great News!</b> Your withdrawal request for <b>Rs.{format_amount(amount)}</b> has been approved."
        NotificationHelper.create_notification(session, shop.owner_id, message)
    ...
    def notify_withdrawal_rejected(
        ...
        message = f"Your withdrawal request for <b>Rs.{format_amount(amount)}</b> has been rejected."
        if reason:
            message += f" Reason: {reason}"
        NotificationHelper.create_notification(session, shop.owner_id, message)
    ...
    def notify_withdrawal_processed(session: Session, shop_id: int, amount: float, net_amount: float):
        ...
        message = f"<b>Payment Completed!</b> Your withdrawal of <b>Rs.{format_amount(net_amount)}</b> has been processed and transferred to your account."
        NotificationHelper.create_notification(session, shop.owner_id, message)
