...
def calculate_grouped_product_price(grouped_products: List[GroupedProductItem], 
                                  config: GroupedProductConfig,
                                  session) -> tuple[float, float, int]:
    """
    ...
    for item in grouped_products:
        ...
        if product:
            ...
            if not item.is_free:  # Free items don't affect available quantity
                item_available = product.quantity // item.quantity if item.quantity > 0 else 0
...
def enhance_grouped_products_data(session, grouped_products: List[Dict], config: Dict) -> tuple[List[Dict], float, float, int]:
    """
    ...
    config_obj = GroupedProductConfig(**config) if config else GroupedProductConfig(
    ...
    final_price, original_price, available_quantity = calculate_grouped_product_price(
        [GroupedProductItem(**item) for item in enhanced_items],
        config_obj,
        session
    )
...
def get_product_with_enhanced_data(session, product_id: int):
    ...
    if product.product_type == ProductType.GROUPED and product.grouped_products:
        enhanced_items, calculated_price, original_price, calculated_quantity = enhance_grouped_products_data(
    ...
    if product.product_type == ProductType.VARIABLE:
        ...
        product_data.quantity = total_quantity  # Update main quantity with sum of variations
...
def create(
    ...
    if request.product_type == ProductType.SIMPLE:
        ...
        enhanced_items, final_price, original_price, available_quantity = enhance_grouped_products_data(
            session, 
            [item.model_dump() for item in request.grouped_products],
            request.grouped_products_config.model_dump() if request.grouped_products_config else None
        )
        ...
        grouped_config_data = request.grouped_products_config.model_dump() if request.grouped_products_config else None
    else:
        min_price = request.min_price or request.price
        max_price = request.max_price or request.price
        total_quantity = request.quantity or 0
        grouped_products_data = [item.model_dump() for item in request.grouped_products] if request.grouped_products else None
        grouped_config_data = request.grouped_products_config.model_dump() if request.grouped_products_config else None
    
    # Prepare product data
    product_data = request.model_dump(exclude={'attributes', 'variations', 'grouped_products', 'grouped_products_config'})
    product_data.update({
        'min_price': min_price,
        'max_price': max_price,
...
def update(
    ...
    if request.product_type == ProductType.VARIABLE and request.variations:
        ...
        # Calculate grouped product pricing and availability
        enhanced_items, final_price, original_price, calculated_quantity = enhance_grouped_products_data(
            session, 
            [item.model_dump() for item in request.grouped_products],
            request.grouped_products_config.model_dump() if request.grouped_products_config else None
        )
    ...
    # Prepare update data
    update_data = request.model_dump(exclude_none=True, exclude={'attributes', 'variations', 'grouped_products', 'grouped_products_config'})
    ...
    if request.grouped_products_config is not None:
        update_data['grouped_products_config'] = request.grouped_products_config.model_dump()
...
def get_group_availability(id: int, session: GetSession):
    ...
    if not product.grouped_products:
        return api_response(200, "Group availability", {
            "available": False,
    ...
    message = "Available for purchase" if can_sell else f"Insufficient stock for: {', '.join(limiting_products)}"
...
def sync_variable_product_quantity(id: int, session: GetSession):
    ...
    return api_response(200, "Quantity synced successfully", {
